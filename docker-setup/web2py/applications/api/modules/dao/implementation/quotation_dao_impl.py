...
class QuotationDAOImplementation(QuotationDAO):
    ...
    def insert_single_quotation(self,data,curr_bill_no):
        ...
        db(data.quotation.user_id==data["user_id"] and db.quotation.bill_no ==  curr_bill_no).update(isActive=1)        
    
    def insert_multiple_quotation(self,data,curr_bill_no):
        """
    ...
    def get_auth_user_quotation_data(self,user_id):
        """
        ...
        db = current.db
        auth = current.auth
        quotation_data = db(db.quotation.user_id == user_id).select(db.quotation.isActive,db.quotation.bill_no,db.quotation.company,db.quotation.purchase_date,groupby=db.quotation.purchase_date)
        return quotation_data

    def get_max_quote_bill_no(self, user_id):
        """
        ...
        db = current.db
        auth = current.auth
        max_bill_number = db(db.quotation.user_id == user_id).select(db.quotation.bill_no.max())[0]["MAX(quotation.bill_no)"]
        return max_bill_number

    def get_generate_bill_data(self,user_id, bill_no):
        """
        Returns bill_data corresponding to current bill_no from quotation table.
        """
        db = current.db
        auth = current.auth
        data = db(db.quotation.user_id == user_id and db.quotation.bill_no == bill_no).select()
        return data

    def get_auth_user_bill_quote(self,user_id, bill_no):
        """
    ...
    def insert2sale_single(self,data,curr_bill_no):
        ...
        db=current.db
        auth=current.auth
        stock_count = db(db.stock.user_id == data[0]["user_id"] and db.stock.description == data[0]["description"]).select() 
        if self.can_be_sold_single(len(stock_count),int(data[0]["quantity"])):
            ...
            db(db.quotation.user_id == data[0]["user_id"] and db.quotation.bill_no == data[0]["bill_no"]).update(isActive=0)
            if len(stock_count) == 0:
                db.stock.insert(user_id = data[0]["user_id"],description =data[0]["description"],quantity=data[0]["quantity"])
            else:
                db(db.stock.user_id == data[0]["user_id"] and db.stock.description == data[0]["description"]).update(quantity=(int(stock_count[0].quantity) - int(data[0]["quantity"])))
            return True
        else:
            return False
    ...
    def insert2sale_multiple(self,data,curr_bill_no):
        ...
        if self.can_be_sold_multiple(data):   
            for i in range(total):                
                stock_count_row = db(db.stock.user_id == data[i]["user_id"] and db.stock.description == data[i]["description"]).select()
                stock_count = int(stock_count_row[0]["quantity"])    
                ...
                db(db.stock.user_id == data[i]["user_id"] and db.stock.description == data[i]["description"]).update(quantity=stock_count - int(data[i]["quantity"]))
        else:
            return False
        db(db.quotation.user_id == data[i]["user_id"] and db.quotation.bill_no == data[0]["bill_no"]).update(isActive=0)
        return True