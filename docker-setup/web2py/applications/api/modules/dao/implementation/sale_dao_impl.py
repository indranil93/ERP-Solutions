...
class SaleDAOImplementation(SaleDAO):
    ...
    def get_sales_history_data(self, user_id):
        db = current.db
        auth = current.auth
        sales_history = db(db.sales_materials.user_id == user_id).select(db.sales_materials.amount,db.sales_materials.purchase_date,groupby=db.sales_materials.purchase_date, orderby=db.sales_materials.purchase_date)
        return sales_history

    def get_trending_purchaser(self, user_id):
        db = current.db
        auth = current.auth
        purchaser_items = db(db.purchase_materials.user_id == user_id).select(join=db.company.on(db.purchase_materials.company == db.company.id), orderby=~db.purchase_materials.purchase_date, groupby=db.purchase_materials.company, limitby=(0,5))
        return purchaser_items

    def get_auth_user_sale_data_period(self,data):
        db=current.db
        auth = current.auth
        sale_data = db(db.sales_materials.user_id == data["user_id"] and db.sales_materials.purchase_date >= data["from_date"] and db.sales_materials.purchase_date <= data["to_date"]).select()
        return sale_data
